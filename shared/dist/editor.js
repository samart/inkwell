...
import { blockquoteSchema, wrapInBlockquoteCommand, isNodeSelectedCommand, headingSchema, wrapInHeadingCommand, codeBlockSchema, createCodeBlockCommand, } from '@milkdown/kit/preset/commonmark';
...
export class MarkdownEditor {
                ...
                [CrepeFeature.ImageBlock]: {
                    onUpload: async (file) => {
                        ...
                        try {
                            ...
                            self.options.onError?.('Failed to upload image: ' + error.message);
                            return '';
                        }
                    },
                },
    ...
    async handlePaste(event) {
        ...
        if (!items || !this.options.onImageUpload)
            return;
    ...
    async handleDrop(event) {
        ...
        if (!files || files.length === 0 || !this.options.onImageUpload)
            return;
